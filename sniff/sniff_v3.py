...
def test_for_shellcode((appPort, attackPort, direction, payload, file_path)):
    try:
        ...
        current_log.info(direction + ": " + "\n[+] Payload received: " + color.YELLOW + payload.replace("\n","\\n") + " (" + payload.encode("hex") + ")" + color.END)
        for keyword in BLACKLIST_KEYWORDS[appPort]:
            if keyword in payload:
                # print colored(("Detect %s in payload, check %s\%s.log\n" % (keyword, str(appPort), str(attackPort))),'red')
                MAIN_LOG.info("\n[*] Detect " + color.RED + keyword + color.END + " in payload, check " + color.RED + "./" + str(appPort) + "/" + str(attackPort) + ".log" + color.END)
